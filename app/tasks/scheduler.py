...
async def cleanup_expired_captcha_sessions():
    ...
    try:
        ...
        timeout_threshold = datetime.now(settings.TIMEZONE) - timedelta(minutes=3)
        ...
        for session in expired_sessions:
            ...
            if session.createdAt.tzinfo is None:
                session.createdAt = settings.TIMEZONE.localize(session.createdAt)
            ...
            print(f"Captcha session {session.id} (client_token: {session.clientToken}) timed out and logged.")
        db.commit()
    except Exception as e:
        db.rollback()
        print(f"Error during captcha cleanup: {e}")
    finally:
        db.close()

def start_scheduler():
    scheduler.add_job(cleanup_expired_captcha_sessions, 'interval', minutes=1) # 1분마다 실행
    scheduler.start()
