...
class EventData(BaseModel):
    ...
    t: int = Field(..., description="상대 타임스탬프 (밀리초)")
    ...
    target_role: Optional[str] = Field(None, description="클릭 대상 역할 (예: answer-1, canvas-container)")
    target_answer: Optional[str] = Field(None, description="클릭 대상 답안 (예: A, B, C, D)")
    payload: Optional[Dict[str, Any]] = Field(None, description="추가 데이터 (예: moves 타입에서 사용되는 dts, xrs, yrs)")
...
class EventChunk(BaseModel):
    ...
    meta: Dict[str, Any] = Field(..., description="이벤트 메타 정보 (예: device, viewport)")
