...
def download_and_parse_session(client, bucket, key):
    ...
    try:
        ...
        print(f"오류: S3 버킷 '{bucket}'에서 파일 '{key}'를 찾을 수 없습니다.", file=sys.stderr)
        return None
...
def get_new_client_token(problem_url, api_key):
    ...
    try:
        ...
        print(f"응답: {e.response.text if e.response else 'N/A'}", file=sys.stderr)
        return None
    except KeyError:
...
def submit_for_verification(verify_url, api_key, client_token, behavior_data):
    ...
    try:
        ...
        response = requests.post(verify_url, headers=headers, json=request_body)
        response.raise_for_status()
...
def poll_for_result(result_url_base, task_id, max_retries=30, interval=2):
    ...
    print(f"'{result_url}'에서 최종 결과 폴링 시작...", end="", flush=True, file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="S3에서 캡챠 세션을 리플레이하고 API로 검증합니다.")
    parser.add_argument("s3_filename", help="다운로드할 S3 파일 이름 (예: 20250908-092649_yaz5q1pdlm.json.gz)")
    parser.add_argument("--type", choices=['human', 'bot'], required=True, help="데이터 타입 (human 또는 bot)")
    parser.add_argument("--problem-url", default="http://localhost:8001/api/captcha/problem", help="캡챠 문제 요청 API의 전체 URL")
    parser.add_argument("--verify-url", default="http://localhost:8001/api/captcha/verify", help="캡챠 검증 요청 API의 전체 URL")
    parser.add_argument("--result-url-base", default="http://localhost:8001/api/captcha/verify/result", help="캡챠 결과 조회 API의 기본 URL")
    ...
    behavior_data = download_and_parse_session(s3_client, s3_config["bucket_name"], s3_key)
    if not behavior_data:
        sys.exit(1)

    # 4. API로 검증 요청 제출
    initial_response = submit_for_verification(args.verify_url, API_KEY, client_token, behavior_data)
